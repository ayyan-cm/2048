...
function moveUp(blockValuesTemp) {
    for (let col = 0; col < 4; col++) {
        let colArray = [blockValuesTemp[0][col], blockValuesTemp[1][col], blockValuesTemp[2][col], blockValuesTemp[3][col]];
...
function moveDown(blockValuesTemp) {
    for (let col = 0; col < 4; col++) {
        let colArray = [blockValuesTemp[0][col], blockValuesTemp[1][col], blockValuesTemp[2][col], blockValuesTemp[3][col]].reverse();
        colArray = move(colArray);
        for (let row = 0; row < 4; row++) {
...
async function handleSwipe() {
    ...
    if (Math.abs(deltaX) > Math.abs(deltaY)) {
        newBlockValues = deltaX > 40 ? moveRight(newBlockValues) : moveLeft(newBlockValues);
    } else {
